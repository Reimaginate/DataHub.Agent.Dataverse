...
public class ProcessDataHubEntityUpdatedNotificationsRequestHandler(IMediator mediator, IOptions<DataverseAgentOptions> dataverseAgentOptions)
    : IHandler<ProcessDataHubEntityUpdatedNotificationsRequest, NullResponse>
{
    public async Task<NullResponse> HandleAsync(ProcessDataHubEntityUpdatedNotificationsRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var dataHubEntityIds = dataHubEntityType.Select(s => s.DataHubEntityId).Distinct().ToList();
